...
public class InvoiceDocument extends BusinessDoc{
	...
	public void extract(int pageNo) throws IOException
	{
		...
		if(tableStartLine >= 0)  //if a table exists on this page, divide the page into preamble, table and summary 
		{
				//Preamble
			...
			preamble = wordExtractor.extract(preambleBoxes, stripper.classifyStyle(preambleBoxes));
			...
			if(tableEndLine!=-1)
			{
				for(int i = this.tableEndLine ; i<numLines ; i++)			//collecting lines from  table end line to end of page
		...
		{	
			...
			preamble = wordExtractor.extract(pageBoxes, stripper.classifyStyle(pageBoxes)); //store everything in preamble 
	...
	{
		//collecting lines relevant to the table (lines from column headings onwards on the first page)			
